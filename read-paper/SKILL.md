...
3. **Stay in this session's scope.** This session reads one paper. Refuse
   ...
   **But the user can end the session at any time.** If they say they're done,
   ...
   lapse — don't push back, don't ask them to finish first. Offer the step 7
...
5. **Never use the AskUserQuestion tool to ask them anything in steps 3–7.**
...
Read the whole thing before step 4. Do not report on it yet.
...
## Step 3 — Orientation: their words first

Before you tell them anything about the paper, they orient themselves. Ask them
to read **only the abstract** — nothing else — and come back with two things:

- **The abstract in one sentence, in their own words.** One sentence, not three.
  Compressing it is the work.
- **Why they want to read this paper.** What they're hoping to get out of it,
  or what made them pick it up.

Stop and wait for both.

Then respond briefly — three or four lines at most:

- If their one-sentence version **missed something central**, say what, in one
  line. If it's good, say so and move on. Don't improve on a summary that
  already works.
- **Reflect their reason back as the frame for the session**, and reconcile it
  with the goal they gave in step 1 if the two differ — their stated reason here
  is usually the more honest one, so prefer it, and say that you're doing so.
  If it changes the depth budget, say how.

This step exists because arriving at a paper with your own framing already in
place is what makes the detail stick. Do not skip it, do not merge it into
step 4, and do not supply the one-sentence summary yourself if they struggle —
ask them for a second attempt, pointing at what the abstract's last sentence
claims.

## Step 4 — Pre-reading questions
...
Build on step 3 rather than restarting: they have just told you what they think
the paper is about and why they care. The question you pose below should connect
to that.
...
## Step 5 — The map: equations, sections, figures
...
## Step 6 — The reading path, one chunk at a time
...
- **First, ask them to state the paper's answer in their own words** — the
  question from step 4, answered. Not the details; the message. Wait. Then show
  them their original guess from step 4 alongside it — and their one-sentence
  abstract from step 3 — and ask what moved.
